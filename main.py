...
@app.get("/converters", response_class=JSONResponse)
async def get_converters():
    converters = [(module.name, module.endpoint) for module in converter_modules.values()]
    return {"converters": converters}

@app.get("/convert/{converter_name}", response_class=HTMLResponse)
...
@app.post("/convert/{converter_name}")
async def convert_file(
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        for temp_file in TempFileGenerator(files):
            try:
                converter_func(str(temp_file), str(Path(tmpdir) / f"converted_{temp_file.name}"))
    ...
    return FileResponse(tempzip.name, media_type='application/x-zip-compressed', filename="converted_files.zip")
